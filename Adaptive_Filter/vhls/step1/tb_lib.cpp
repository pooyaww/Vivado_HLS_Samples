...
// Constructors
tb_lib::tb_lib(bool init_do_write_files, string init_desired_fname, string init_noise_fname, string init_output_fname, adaptive_filter* init_uut, int init_num_samples_to_test)
{
...
void tb_lib::run_test()
{
	...
	for (ii = 0; ii < num_samples_to_test; ii++)
	{
		...
		output[ii] = uut->run_filter(signal_with_noise[ii], noise[ii]);
	}
}
