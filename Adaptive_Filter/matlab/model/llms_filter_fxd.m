...
classdef llms_filter_fxd < adaptive_filter_fxd

	methods (Access = public)
		function obj = llms_filter_fxd()
	...
	methods (Access = protected)
		function coefs = update_coefs(obj, coefs, noise_buf, step_size, leakage, err)
			...
			for ii = 1:length(coefs)
				coefs(ii) = 2^-(obj.filter_params.num_coef_bits-1)*((2^(obj.filter_params.num_coef_bits-1)-1)-floor(leakage*step_size*2^-((obj.filter_params.num_coef_bits-3))))*coefs(ii) + floor(scaling * step_size * err * noise_buf(noise_buf_idx)); % This node is 2x data_width + coef_width
